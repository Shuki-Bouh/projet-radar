...
if __name__ == '__main__':
    ...
    for i in range(data.shape[3]):
        ...
        nb_guard = 1
        nb_ref = 3
        result, conv, mask = sgp.CFAR2D(pfa, nb_guard, nb_ref, fftd_abs_norm_mean1)

        ax.clear()

        # ax.imshow(fftd_abs_norm_mean1, extent=[np.min(speed), np.max(speed), np.min(rang), np.max(rang)],
        #            origin='lower')
