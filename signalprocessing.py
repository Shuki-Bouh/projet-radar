...
def plot_doppler_spectrum(fft2, mode= [None,None]):
    ...
    if mode[0] == 'TDMA':
        ...
        fft2 = DDMA_antenna2_channel(fft2, mode[1])
        Nt =  len(mode[1])
        Nr = fft2.shape[2]//Nt
        
    fft_abs = np.abs(fft2)
    if mode[0] == 'DDMA': #conservation du min selon les channels de la meme antenne Tx, pour enlever les pics fantomes
